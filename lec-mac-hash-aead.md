...
- $k$ 回ランダムな値のハッシュ値を計算する
  ...
  - $P_3=1-\texttt{（全部異なる確率）}=1-((N-1)/N× (N-2)/N)$
...
# 有限体
## 体 (field) とは四則演算のできる集合
- 後の講義でもう少し詳しく説明
## 有限体
- 有限個の要素からなる体
## $𝔽_2$
- 0と1からなる最小の有限体
- 加算（排他的論理和）: $0+0=0$, $0+1=1+0=1$, $1+1=0$
- 乗算（論理積）: $0\cdot 0=0$, $0\cdot 1=1\cdot 0=0$, $1\cdot 1=1$
## $n$ 次拡大体
- 体 $K$ の要素を $n$ 個並べたもの $L=K^n$ に新しい四則演算を定義して体にしたもの
...
- $L:=K[x]/(f(x))=\Set{a+bx \mid a, b \in K}$ は $K$ の2次拡大体
  - 注意: $𝔽_5$ 上では $x^2+1=(x-2)(x+2)$ なので $𝔽_5[x]/(x^2+1)$ は体にならない
    - $x-2$ や $x+2$ の逆元（逆数）が存在しない
## $K:=𝔽_2$ の2次拡大体
