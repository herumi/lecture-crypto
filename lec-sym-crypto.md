...
## 定義
- 1bit変数 $a$ と $b$ の排他的論理和（以降xorと略す） $a ⊕ b$ は次の表で定義される
...
それをまとめて $c = Enc(s, m) = m ⊕ s$ と書く
- 復号はビットごとに暗号文 $c$ と乱数 $s$ のxorをとる
$Dec(s, c)=m$
  - xorの性質により $Dec(s, Enc(s, m))=(m ⊕ s) ⊕ s = m$
## 特徴
- 平文と秘密鍵の大きさが同じ
- 情報理論的安全性（次のスライド）を持つ
...
- 同じ秘密鍵で複数回使うと安全性が保てない
  - $c_1=m_1 ⊕ s$, $c_2=m_2 ⊕ s$ とする（単純化のために1bitで考える）と
...
- ストリーム暗号 $Enc(s,m):=(r, F(s,r) ⊕ m)$, ここで $r$ はランダムに選んだ値
...
- 入力: 256bitの秘密鍵 $k$ と96bitのナンス $n$
  - ナンス (nonce): 一度だけ使われる値
  - ナンスが64bitは小さい, 96bitだとAES-GCMと同じ長さで扱いやすい
...
- 暗号文と一緒にIVも送る
- $c_1=Enc(s, m_1 ⊕ IV)$
- $c_k=Enc(s, m_k ⊕ c_{k-1})$ for $k \ge 2$.
## 特徴
...
- $f(x) = o(g(n))$ であるとは $\lim_{x → ∞} f(x)/g(x) = 0$ であること
...
- クラス $P$ は証拠を多項式時間で検証できるのでクラス $NP$ に含まれる. $P ⊆ NP$
- 「多項式時間で解けない問題の集合」*ではない*

## SAT (Satisfiability Problem)
- 与えられた論理式 $φ(x)$ について $φ(a)=1$ となる $a$ は存在するか?
  - 証拠 $a$ が $φ(a)=1$ となるのを確認するのは多項式時間なので クラス $NP$ に属する
