...
- A は $x ∈ [1, p-1]$ をランダムに選んで $X:=g^x \bmod{p}$ （$p$ で割った余り）を B に送る
- B は $y ∈ [1, p-1]$ をランダムに選んで $Y:=g^y \bmod{p}$ を A に送る
...
- 計算コストは $L_p[1/3, (64/9)^{1/3}]$
  ...
  - $f(x)$ が $o(g(n))$ であるとは $\lim_{x → ∞} f(x)/g(x) = 0$ であること
  - $L_p[1, c] ∼ p^c$ （$\log p$ の指数時間）, $L_p[0, c] ∼ (\log p)^c$ （$\log p$ の多項式時間）
...
# ECDH鍵共有
## DH鍵共有の類推
- A は $x ∈ [1, r-1]$ をランダムに選んで $X:= x P$ を B に送る
- B は $y ∈ [1, r-1]$ をランダムに選んで $Y:= y P$ を A に送る
...
- $f:G \times G → G$ が存在し, $f(a,b)$ を $a \cdot b$ と書く（$a b$ と書くことも）
  - 結合則: $(a b) c = a (b c)$ for $∀ a, b, c ∈ G$
  - 単位元: $e ∈ G$ が存在して$e a = a e = a$ （$e=1$ と書くことがある）
  - 逆元: $∀a ∈ G$ に対して $a^{-1} ∈ G$ が存在して $a a^{-1} = a^{-1} a = e$
## 可換群
- 群 $G$ について $∀a, b ∈ G$ に対して $a b = b a$ が成り立つ
- このとき $a b$ を $a + b$ と書き, 単位元を $e=0$, 逆元を $-a$ と書くことがある（加法群という）

# 群の例
## 整数の集合 $G:=ℤ$
- $a, b ∈ G$ に対して $f(a,b):=a+b$, 0が単位元の可換群
## 0以外の有理数の集合 $G:=ℚ^*$
- $a, b ∈ G$ に対して $f(a,b):=a b$, 1が単位元の可換群
## 2行2列の実数行列で逆行列を持つ行列の集合 $G:=GL_2(ℝ)$
- $a, b ∈ G$ に対して $f(a,b):=a b$ （行列の積）,  単位行列が単位元の非可換な群
...
# 巡回群
## 群 $G$ が巡回群であるとは
- $g ∈ G$ に対して $⟨g⟩:= \Set{e, g, g^2, g^3, \ldots}$ を $g$ で生成される部分群（巡回群）という
  - $⟨e⟩ = \Set{e}$.
- $g^r=e$ となる $r$ が存在すれば $⟨g⟩ = \Set{e, g, g^2, \ldots, g^{r-1}}$ となり有限巡回群という
  - $r$ を $⟨g⟩$ の位数といい $r = |⟨g⟩|$ と書く
- 巡回群は可換群である
  - $a, b ∈ ⟨g⟩$ に対して $a=g^x$, $b=g^y$ と書けて $a b = g^{x+y} = g^{y+x} = b a$
## 有限巡回群 $G=⟨g⟩$ に関するDH鍵共有
- $x, y ∈ [1, r-1]$ をランダムに選んで $X=g^x$, $Y=g^y$ を交換し $g^{xy}$ を共有する
- DH鍵共有とECDHの鍵共有は $f(a,b)$ を $a b$ と書くか $a + b$ と書くかの違いだけ
- 実際論文によっては楕円曲線の演算を $g^a$ の形で書いてあるものもある（講義では $+$ を使う）

# ρ法
<!-- _class: image-right-center -->
![w:400px](images/lec-rho-method.drawio.svg)
## 位数 $r$ の加法巡回群 $G=⟨P⟩$ のDLPに対する一般的な攻撃
- $Q=aP∈ G$ となる $a$ を見つけたい
- $m$: 分割数を固定し, 整数 $(u_i,v_i)$ をランダムに選び
$M_i:=u_i P + v_i Q$ とする（事前計算）
$g(R):=R_x \bmod{m}$ とする
- $R_0:=P$, $R_{i+1}:=R_i + M_{g(R_i)}$ と順次求める
  - $R_i = a_i P + b_i Q$ の形
- $R_i = R_j$ となる $i<j$ が見つかったとする（衝突）
  - $R_i = a_i P + b_i Q = a_j P + b_j Q = R_j$
  - $Q=(a_i - a_j)/(b_j - b_i) P$ # $b_i = b_j$ ならやり直し
- 衝突するのに必要なステップ数は $O(\sqrt{r})$
- 今（2025年）のところこれより効率よくECDLPを解く方法は知られていない
- $r$ bitのECDLPの安全性は $r/2$ bit

# 楕円曲線のためにしばらく数学の準備
## 合同
- 整数 $m(>0)$, $a$, $b$ に対して $a ≡ b \pmod{m}$ とは $(a-b) \bmod{p} = 0$ のときをいう
...
- 加減算: $a \pm b := (a \pm b) \bmod{p}$. 乗法: $a b := (a b) \bmod{p}$ for $a, b ∈ ℤ/m$
...
## 逆数とは
- $ax=1$ となる $x$ が $a$ の逆数
- 整数環でも $ab ≡ 1 \pmod{m}$ となる $b$ があれば $b$ を $a$ の逆数と呼んでよいのでは

- 例えば $m=7$ のとき $3 \times 5 ≡ 1 \pmod{7}$ なので $3$ の逆数 $1/3=5$ とする
- 例えば $m=6$ のとき $3 \times x \not≡ 1 \pmod{6}$ なので $3$ の逆数は存在しない
  - $3 \times 1 ≡ 3$, $3 \times 2 ≡ 0$, $3 \times 3 ≡ 3$, $3 \times 4 ≡ 0$, $3 \times 5 ≡ 3$
# 有限体
## 素数 $p$ に対する整数環
- $𝔽_p := ℤ/p = \{0, 1, 2, \ldots, p-1\}$
- **定理**: $∀ a ∈ {𝔽_p}^*:=𝔽_p \setminus \{0\}$ に対して $a$ の逆数が存在する
  - つまり $a b ≡ 1 \pmod{p}$ となる $b ∈ {𝔽_p}^*$ が存在する
- $𝔽_p$ は四則演算ができる
  - 逆元の存在: $a ∈ {𝔽_p}^*$ に対して $a^{-1} ∈ {𝔽_p}^*$ が存在して $a a^{-1} = 1$
...
- $∀ a ∈ {𝔽_p}^*$ に対して $a$ と $p$ は互いに素である（$p$ は素数だから）
  - つまり $\gcd(a, p)=1$
- 互除法を用いて $a x + p y = \gcd(a,p)=1$ となる整数 $x$, $y$ が存在する
- これは $a x ≡ 1 \pmod{p}$, つまり $x \bmod{p}$ は $a$ の逆数
## $ℤ/m$ が有限体になる条件
- $m$ が合成数 $m=uv$ ($u, v > 1$) のとき, $u v ≡ 0 \pmod{m}$ なので $u$ の逆数は存在しない
...
- $(x+y)^n = \sum_{k=0}^{n} {n \choose k} x^k y^{n-k}$ が成り立つ
  - 問題: 数学的帰納法を使って証明せよ
  - 例: $(x+y)^5=x^5 + 5 x^4 y + 10 x^3 y^2 + 10 x^2 y^3 + 5 x y^4 + y^5$
- 素数 $p$ と $0 < k < p$ に対して ${p \choose k}$ は $p$ の倍数
  - ${p \choose k} = \frac{p!}{k!(p-k)!}$ の分子に $p$ があり, 分母に $p$ がない
- $(x+y)^p ≡ x^p + y^p \pmod{p}$
  - $(x+y)^p = \sum_{k=0}^{p} {p \choose k} x^k y^{p-k} ≡ x^p + y^p \pmod{p}$
- $x^p ≡ x \pmod{p}$
  - $x^p ≡ (1+(x-1))^p ≡ 1 + (x-1)^p ≡ 1 + \cdots + 1 ≡ x \pmod{p}$

# Fermatの小定理
## 素数 $p$ と $a∈ {𝔽_p}^*$ に対して $a^{p-1} ≡ 1 \pmod{p}$
- $a^p ≡ a \pmod{p}$ より $a(a^{p-1}-1) ≡ 0 \pmod{p}$
- $a$ と $p$ は互いに素なので $a^{p-1}-1 ≡ 0 \pmod{p}$.
## 逆元の計算
- Fermatの小定理より $a ∈ {𝔽_p}^*$ に対して $a (a^{p-2}) ≡ 1 \pmod{p}$.
...
## 体 $K$ 上の（Weierstrass型）楕円曲線 $E/K$ とは
- $E(K):=\Set{(x,y) ∈ K^2 \mid y^2=x^3+a x + b} \cup \Set{O}$
  - 条件: $a, b ∈ K$ で $4 a^3 + 27 b^2 ≠ 0$ （$x^3+ax+b=0$ が重解を持たない）
## 実数上のグラフ
- $E$ の定義方程式 $y^2=x^3+a x + b$ のグラフと $O$ の集合
- $x$ 軸に対して対象
- $O$ をシンボル的に無限遠点とする（$(0,0)$ ではない）
## 楕円曲線に加算を定義する
- 一般の $P, Q ∈ E(K)$ について $P+Q ∈ E(K)$ を次で定義する:
$P$ と $Q$ を通る直線 $l_1$ と $E(K)$ の交点 $R'$ をとり,
$R'$ の $y$ 座標の符号を反転させた点を $R:=P+Q$ とする

# 定義概要
## 特別な点や関係のとき
- $P + O := O + P := P$ for $∀ P ∈ E(K)$, $-O:=O$
- $P=(x,y)$ について $-P:=(x,-y)$
- $P=Q$ のとき $P$ における接線を考える
- $Q=-P$ のとき直線は $y$ 軸に並行となり $P+(-P):=O$ とする
## $G:=E(K)$ が群であることの確認
- 単位元と逆元は定義から自明
- 結合則: $(P+Q)+R = P+(Q+R)$ for $∀ P, Q, R ∈ E(K)$
...
# 楕円曲線の演算の定義まとめ
## 単位元
- 任意の $P ∈ E(K)$ に対して $P+O=O+P=P$
## 逆元
- $-O:=O$
任意の $P:=(x,y) ∈ E(K) \setminus \Set{O}$ に対して $-P:=(x,-y)$, $P+(-P)=O$
## 加法
...
## mod pのDH鍵共有
- $p$ を大きな素数, $P ∈ [1, p-1]$ をランダムに固定・共有し
$a ∈ [1, p-1]$ をランダムに選び $a P$ を相手に送る方式のDH鍵共有は安全か
...
- 鍵生成 $1^λ$ を入力とし署名鍵 $sk$ と検証鍵 $vk$ を出力
  - $Gen(1^λ)=(sk, vk)$
  - メッセージ空間 ${\cal M}$ も決まる
- 署名生成 $sk$ と $m ∈ {\cal M}$ から署名 $σ$ を出力
  - $Sig(sk, m)=σ$
  - 暗に $vk$ を入力することもある
- 検証 $(vk,m,σ)$ を入力し正当なら1, 不正なら0を出力
  - $Ver(vk, m, σ) ∈ \Set{0,1}$
### 補足
- 署名は $m$ を平文のまま送るので $m$ の機密性は別途暗号化により担保する
...
## 正当性
- $Ver(vk, m, Sig(sk, m))=1$.
  - 正しい署名鍵を使って署名すれば正しいと判定される

## 偽造できない
- 署名鍵を持っている人しか正しい署名を作れない
- 偽のメッセージに対する署名を作れない
- 否認できない
  - MACは $sk=vk$ だったのでAとBの両方が正当なMAC値を生成できる
    - 互いに「相手がMAC値を作った」と主張できる
...
- $Sig(s, m)$: $k \underset{U}\leftarrow {𝔽_r}^*$ を選び $R := k P$ の $x$ 座標を $t$ として（$t=(kP)_x$ と書くことにする）
$σ:=(t, (H(m)+s t)/k \bmod{r})$ を署名とする
- $Ver(S, m, σ=(t, u))$:
$P':=(H(m) P + t S)/u$ の $x$ 座標が $t$ ならば1, それ以外は0を出力
  - 分母が0になる場合はやり直す
  - 正当性: $P'=(H(m) P + t s P)/((H(m)+s t)/k)=k P=R$ なので $t=P'_x$
## 注意
- 署名時の乱数は予測できてはならない
- 2010年PlayStation 3の署名は乱数を固定値にしていたため秘密鍵が漏洩した

# 固定乱数を利用したときのECDSAの危険性
## 署名 $Sign(s,m_i;k_i)=σ_i=(t_i, u_i)$ ($i=1,2$) を考える
- $u_1=(H(m_1)+s t_1)/k_1$
- $u_2=(H(m_2)+s t_2)/k_2$ ; $u_i$, $H(m_i)$, $t_i$ は既知, $s$ は未知
  - $k_1$, $k_2$ が未知でもその差 $k_2 - k_1 = d$ が既知とすると変数2個で方程式2個なので解ける
- $k_1 u_1 - s t_1 = H(m_1)$
- $k_1 u_2 - s t_2 = H(m_2) - d u_2$
- 特に $k_1 = k_2$ ($d=0$) のとき解ける. 乱数が単調増加 (e.g., $d=1$) していても解ける
## 乱数の代わりに決定的アルゴリズムを用いる手法 [RFC-6979](https://datatracker.ietf.org/doc/html/rfc6979)
- $k$ を秘密鍵 $s$ とメッセージのハッシュ値 $h=H(m)$ から決定的に生成する
- $V:=\texttt{0x0101...01}$, $K:=\texttt{0x0000...00}$
- $K=HMAC_K(V \| \texttt{0x00} \| s \| h)$, $V=HMAC_K(V)$
- $K=HMAC_K(V \| \texttt{0x01} \| s \| h)$, $V=HMAC_K(V)$, ...（省略）により $k$ を出力
...
## バイナリ法
- $n$ を2進数で表現する $n=\sum_{i=0}^{L-1} n_i 2^i$. ($n_i ∈ \{0,1\}$, $L \approx \log_2 r \approx 256$)
...
## バイナリ法の改善
- $w$ bitずつまとめて処理する
- 事前に $P, 2 P, 3 P, \cdots, (2^w-1) P$ を計算しておく
- $n$ を $2^w$ 進数展開する
$n= \sum_{i=0}^{\lceil L/w \rceil -1} n_i (2^w)^i$ ($n_i ∈ [0, 2^w-1]$)
- $Q=0$ を初期値として $Q = 2^w Q + n_i P$ を繰り返す
  - $n_i P$ はテーブル参照
## 演算コスト
- 2倍算: $L-1$ 回（バイナリ法と同じ）
- 加算: $(2^w-1) + \lceil L/w \rceil$ 回
  - $L=256$, $w=4$ ならウィンドウの方がコストが小さい

# 射影座標
## 有限体の除算を避けることによる高速化
- $(x,y)$ の形（アフィン座標）では演算のたびに $λ$ の計算で除算が必要
  - 除算を避けたい: 分数 $a/b+c/d=(a d + b c)/(b d)$ を分子・分母のペアとして表現する
  - $x=X/Z$, $y=Y/Z$ として $[X:Y:Z]$ の形（射影座標）で表現する
  - $X'=c X$, $Y'=c Y$, $Z'=c Z$ ($c≠0$) のときも $x=X'/Z'$, $y=Y'/Z'$ なので同じ点
  - $[0:0:0]$ は除外する
- 方程式 $y^2=x^3 + a x + b$ は $Y^2 Z = X^3 + a X Z^2 + b Z^3$ の形(D)
## 無限遠点
- アフィン座標では $O$ という無限遠点が別途表現する必要があった
- $[X:Y:Z]=[0:1:0]$ は(D)を満たすのでこれを無限遠点として統一的に扱う

# 射影座標による加法公式
## $P=[X_1:Y_1:Z_1]$, $Q=[X_2:Y_2:Z_2]$ とする
- $x_1≠x_2$ とする
$λ=(y_2-y_1)/(x_2-x_1)=(Y_2 Z_1 - Y_1 Z_2)/(X_2 Z_1 - X_1 Z_2)=:S/T$
- $X_3=λ^2 - x_1 - x_2 = (Y_2 Z_1 - Y_1 Z_2)^2/(X_2 Z_1 - X_1 Z_2)^2 - X_1/Z_1 - X_2/Z_2$
$X_3 = S^2/T^2 - (X_1/Z_1+X_2/Z_2)=(S^2 Z_1 Z_2 - (X_1 Z_2 + X_2 Z_1))/(T^2 Z_1 Z_2)$
$Y_3$ も同様に計算できる
- アフィン座標に比べて乗算回数は多少増えるが除算が不要になるので高速化できる
- ECDHなどを射影座標のまま計算し, 最後に一度だけアフィン座標に変換する

# サイドチャネル攻撃
<span class="any" style="top:3em;right:1em;">![w:350px](images/lec-side-channel-attack.png)</span>
## 暗号装置が動作中の挙動を観察して解読する攻撃
- 電流・時間・音声・電磁波などを利用
- 電力解析: 消費電力の変化から内部状態を推測
- タイミング攻撃: 処理時間の違いから内部状態を推測
## 楕円曲線のスカラー倍算
- バイナリ法は加算するしないは秘密鍵のビットに対応（情報が漏洩する可能性）

## [EUCLEAK攻撃](https://www.yubico.com/support/security-advisories/ysa-2024-03/) (2024)
- YubicoのYubiKey 5/FIDO2デバイスへの攻撃報告
- InfineonライブラリのECDSAの実装に問題
  - 逆数計算のための拡張Euclid互除法への攻撃
  - 攻撃条件が高いため直ちに脅威にはならない
<span class="any" style="right:0.5em; bottom:2em">![w:550px](images/lec-eucleak.png)</span>
<!--
## 2021年GoogleのTitanセキュリティキーへの[攻撃報告](https://ninjalab.io/a-side-journey-to-titan/)
- FIDO U2F 直ちに脅威となるものではないが（画像は上記リンク先より引用）
<span class="any" style="bottom:0.0em; left:0.5em;">![w:350px](images/lec-titan.png)![w:350px](images/lec-titan-prob.png)![w:350px](images/lec-titan-leak.png)</span>
-->

# EdDSA
## ed25519曲線
- twisted Edwards型楕円曲線
  - $-x^2+y^2=1+d x^2 y^2$, $p=2^{255}-19$, $d=-121665/121666 ∈ 𝔽_p$
  - $O=(0,1)$, $-P=(-x,y)$
## 完全加法公式
- $P_1=(x_1,y_1)$, $P_2=(x_2,y_2)$ に対して $P_3=(x_3,y_3)=P_1+P_2$ は
$x_3=(x_1 y_2 + y_1 x_2)/(1 + d x_1 x_2 y_1 y_2)$,
$y_3=(y_1 y_2 + x_1 x_2)/(1 - d x_1 x_2 y_1 y_2)$
  - $P_1+(0,1)=(x_1,y_1)$, $P_1+(-P_1)=(0,1)$ が成り立つことを確認できる
  - Wierstrass型と違って場合分けが不要

# Curve25519曲線
## Montgomery型楕円曲線
- $v^2 = u^3 + A u^2 + u$, $p=2^{255}-19$, $A=486662$
- $O$ は無限遠点, $P=(u,v)$ に対して $-P=(u,-v)$
- ed25519とCurve25519は互いに変換できる
  - $u=(1+y)/(1-y)$, $v=(\sqrt{A+2})(u/x)$ とするとed25519型になる
  - $d=-(A-2)/(A+2)$
## 加法公式
- Weierstrass型と同様に場合分けが必要
- しかしスカラー倍算は $x$ 座標だけで計算できる方法がある (Montgomery ladder)
  - ECDH鍵共有では $x$ 座標だけ利用するので効率がよい
...
## $f_e$ の性質
- $∀ x ∈ [0, n-1]$ に対して $f_d(f_e(x))=f_e(f_d(x))=x$. すなわち $f_d(x)=f_e^{-1}(x)$
...
$x^{p-1} - 1$ は $p$ で割れ, $x^{q-1} - 1$ は $q$ で割れる
  ...
  - $p$ と $q$ は互いに素なので $x^{(p-1)(q-1)} - 1$ は $n=pq$ で割れる: $x^{(p-1)(q-1)} ≡ 1 \pmod{n}$
  - $d e ≡ 1 \pmod{(p-1)(q-1)}$ より, ある整数 $a$ を使って $d e = 1 + a(p-1)(q-1)$ とすると $x^{e d} = x^{1 + a(p-1)(q-1)} = x (x^{(p-1)(q-1)})^a ≡ x \pmod{n}$
- $x$ が $p$ で割れるとき $x^{e d} ≡ 0 ≡ x \pmod{p}$
  - $x$ が $q$ でも割れるならそれは $x=0$ なので $x^{e d}=0 ≡ x \pmod{n}$
  - $x$ が $q$ で割れないとすると上記と同様に $x^{e d}≡ x \pmod{q}$ なので $x^{e d} ≡ x \pmod{n}$
# RSA仮定
