...
- 計算コストは $L_p[1/3, (64/9)^{1/3}]$
  ...
  - $f(x)$ が $o(g(n))$ であるとは $\lim_{x → \infty} f(x)/g(x) = 0$ であること
...
- どちらも群構造を利用している
## 群とは集合 $G$ が次の性質を満たすもの
- $f:G \times G → G$ が存在し, $f(a,b)$ を $a \cdot b$ と書く（$a b$ と書くことも）
...
## 整数環
- 整数 $m(>0)$ に対して集合 $ℤ/mℤ := \{0, 1, 2, \ldots, m-1\}$ とする（$ℤ/m$ と略記する）
- 加減算: $a \pm b := (a \pm b) \bmod{p}$. 乗法: $a b := (a b) \bmod{p}$ for $a, b \in ℤ/m$
...
# 除算の定義
## 思考
- $ℤ/m \ni a, b$ $(≠ 0)$ に対して除算 $a/b$ を定義したいが$a/b$ は整数にならない場合がある
- まず逆数 $1/a$ を考える
## 逆数とは
- $ax=1$ となる $x$ が $a$ の逆数
...
# 有限体
## 素数 $p$ に対する整数環
- $𝔽_p := ℤ/p = \{0, 1, 2, \ldots, p-1\}$
- **定理**: $\forall a \in {𝔽_p}^*:=𝔽_p \setminus \{0\}$ に対して $a$ の逆数が存在する
  - つまり $a b \equiv 1 \pmod{p}$ となる $b \in {𝔽_p}^*$ が存在する
- $𝔽_p$ は四則演算ができる
  - 逆元の存在: $a \in {𝔽_p}^*$ に対して $a^{-1} \in {𝔽_p}^*$ が存在して $a a^{-1} = 1$
  - 問: 逆元は存在すればただ一つしかないことを示せ
- このとき $𝔽_p$ を有限体という
  - 無限集合だけど四則演算が出来るものを体という
  - 有理数の集合 $ℚ$, 実数の集合 $ℝ$, 複素数の集合 $ℂ$ も体
...
- $b≠ 0$ のとき $\gcd(a,b)=\gcd(a \bmod{b}, b)$.
...
- $f_e:[0,n-1]→ [0,n-1]$ は一方向性置換（全単射）でもある
...
# sshの公開鍵認証
<!-- _class: image-right -->
![w:580px](images/lec-ssh.drawio.svg)
##  署名を用いて本人確認する仕組み
...
- CAは証明書の発行・管理・検証・失効などを行う
 ![w:700px](images/lec-ca.drawio.svg)

# 公開鍵基盤 PKI (Public Key Infrastructure)
<!-- _class: image-right -->
![w:300px](images/lec-ca1.png)
![w:600px](images/lec-ca2.png)
## CAの相互認証
- CAが一つでは権限や責任が集中する
