...
- $𝒫$ が $w$ を知っていることを $w$ を教えずに $𝒱$ に証明
1. $𝒫$ は乱数 $r$ をとって $a:=Com(x,w,r)$ を $𝒱$ に送る
2. $𝒱$ は乱数 $c$ をとって $𝒫$ に送る
3. $𝒫$ は $z:=Resp(x,w,r,c)$ を $𝒱$ に送り
$𝒱$ は $Ver(x,a,c,z)$ で検証する

# $Σ$ プロトコルの性質
- 特別な健全性 (Special Soundness)
  - $(b, h_0, d_0)$ と $(b, h_1, d_1)$ ($h_0\neq h_1$)から $w$ を計算できる
- 特別な正直検証者ゼロ知識 (SHVZK: Special honest Verifier ZK)
  - 検証機が正直に動作し, $h$ を事前に知っていたらシミュレートできる
  - $Σ$ プロトコルをより安全な形に変換する一般的な手法がある
